...
void init_os(CPU& cpu, MEM& mem)
{
    mem.init();
    std::string buf;

    std::cout << "Hi user! what do you want to execute? (you can put lines of assembly or giving the raw binary in .bin)" << std::endl;
    std::cout << ">> ";
    std::getline(std::cin, buf);
    load((char *)buf.c_str(), mem);
    cpu.execute(mem, 0);
}